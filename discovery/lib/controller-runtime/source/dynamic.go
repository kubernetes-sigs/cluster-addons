...
func (d *Dynamic) InformableGVKs() (available []schema.GroupVersionKind, err error) {
	// TODO(njhale): consider extending RESTMapper instead
	...
	informable := discovery.FilteredBy(discovery.SupportsAllVerbs{Verbs: []string{"list", "watch"}}, resourceLists)
	for _, rl := range informable {
