...
// Smoke test for validating CoreDNS operator changes.
// Source: https://github.com/kubernetes-sigs/kubebuilder-declarative-pattern/blob/master/hack/smoketest.go
//
// USAGE:
//   go run smoketest.go
//
...
func verifyNoWorkloadsWithLabel(label, namespace string) error {
	out := mustExec("kubectl", "get", "all", "-l", label, "-n", namespace)
	if out != "" {
